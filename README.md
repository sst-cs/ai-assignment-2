...
- For UCS and A*, the cost of a path is the sum of the edge weights, which you can get using `graph.cost(from_node, to_node)`. 
- For A*, the heuristic value for a node can be accessed via `heuristic[node]`.
